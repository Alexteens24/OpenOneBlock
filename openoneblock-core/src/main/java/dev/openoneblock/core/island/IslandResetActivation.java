...
/**
 * Atomic reset activation request after all starter effects were verified.
 *
 ...
 * @param primarySpawn verified replacement primary spawn
 * @param magicBlock verified replacement Magic Block
 * @param initialPhaseId replacement initial phase
 * @param requiredEffects complete verified world-effect keys
 * @param activatedAt caller clock instant
 */
public record IslandResetActivation(
    IslandId islandId,
    ...
    Instant activatedAt) {
  /** Validates reset activation evidence. */
  public IslandResetActivation {
    ...
    if (!primarySpawn.primary()) {
      throw new IllegalArgumentException("reset activation requires a primary spawn");
    }
    if (!primarySpawn.position().worldId().equals(magicBlock.position().worldId())) {
      throw new IllegalArgumentException("spawn and Magic Block must share one world");
    }
    if (requiredEffects.isEmpty()) {
      throw new IllegalArgumentException("reset activation requires verified world effects");
    }
    for (WorldEffectKey effect : requiredEffects) {
      Objects.requireNonNull(effect, "required effect");
      if (!effect.operationId().equals(operationId)) {
